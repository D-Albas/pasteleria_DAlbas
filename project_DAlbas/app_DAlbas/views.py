...
class ClienteImagen(APIView):
    def post(self,request):
        serializer = ClienteSerializerJson(data=request.data)
        if serializer.is_valid():
            validated_data = serializer.validated_data
            ...
            user.groups.add(rol)
            user.save()
            serializer_response = ClientesSerializer(user)
...
class AdministradorImagen(APIView):
    def post(self,request):
        serializer = AdministradorSerializerJson(data=request.data)
        if serializer.is_valid():
            validated_data = serializer.validated_data
            archivo = validated_data['fotoUsuario']
            archivo.name = 'usuario.png'
            validated_data['fotoUsuario'] = archivo
            user = Administradores(**validated_data)
            user.save()
            serializer_response = AdministradoresSerializer(user)
            return Response(serializer_response.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def registroClienteApi(request, nombre, apellido, identificacion, telefono, correo, contrasena):
    try:
        ...
        if Clientes.objects.filter(email=correo).exists():
        ...
        if Clientes.objects.filter(identificacionCliente=identificacion).exists():
            mensaje = "La identificación ya está registrada en la base de datos."
            return JsonResponse({'mensaje': mensaje, 'estado': False})
        ...
        with transaction.atomic():
            user = Clientes.objects.create(
                identificacionCliente=num_idenf,
        ...
        return JsonResponse({"mensaje": "Error en el registro: " + str(e), "estado": False})
...
def mostrarProductos(request):
    ...
    try:
        ...
        num_categorias = Categorias.objects.count()
        num_tematicas = Tematicas.objects.count()
        ...
        for categoria_info in productos_por_categoria:
            ...
            total_productos_cate = categoria_info['total']
            total_productos_por_categoria[nombre_categoria] = total_productos_cate

        for tematica_info in productos_por_tematica:
            nombre_tematica = tematica_info['tematicaProducto__nombreTematica'].replace(" ", "_")
            total_productos_tema = tematica_info['total']
            total_productos_por_tematica[nombre_tematica] = total_productos_tema
...
def vistaPerfilUsuario(request):
    ...
    if request.user.is_authenticated:
        if request.user.groups.filter(name='Cliente').exists():
            ...
            misPedidos = Pedidos.objects.filter(userPedido=user_id)
            retorno = {"cliente": cliente,"misPedidos": misPedidos}
...
def vistaCarrito(request):
    ...
    if request.user.is_authenticated:
        if request.user.groups.filter(name='Cliente').exists():
            ...
            return render(request, "cliente/carrito.html", {'cantidad_productos': cantidad_productos,'today':today,'cliente': cliente})
        else:
            retorno = {"mensaje": "No tienes permiso para ingresar a esta vista"}
    else:
        retorno = {"mensaje": "Debe ingresar con sus credenciales"}
...
def vistaOtrosRegistros(request):
    if request.user.is_authenticated:
        if request.user.groups.filter(name='Administrador').exists():
            categorias = Categorias.objects.all()
            tematicas = Tematicas.objects.all()
            retorno = {"categorias":categorias,"tematicas":tematicas}
            return render(request, "administrador/categoriasTematicas.html", retorno)
        else:
            retorno = {"mensaje": "No tienes permiso para ingresar a esta vista"}
    else:
        retorno = {"mensaje": "Debe ingresar con sus credenciales"}
...
def registrar_categoria(request):
    if request.method == 'POST':
        ...
        try:
            categoria_existente = Categorias.objects.filter(nombreCategoria=nueva_categoria).first()
            if categoria_existente:
                return JsonResponse({'success': False, 'error': 'La categoría ya está registrada'})
            else:
                categoria = Categorias(nombreCategoria=nueva_categoria)
                categoria.save()
                return JsonResponse({'success': True, 'message': 'Categoría registrada correctamente'})
        except Exception as e:
            return JsonResponse({'success': False, 'error': f'Error al registrar la categoría: {str(e)}'})
    return JsonResponse({'success': False, 'error': 'Solicitud no válida'})

def registrar_tematica(request):
    if request.method == 'POST':
        nueva_tematica = request.POST.get('nueva_tematica')
        try:
            tematica_existente = Tematicas.objects.filter(nombreTematica=nueva_tematica).first()
            if tematica_existente:
                return JsonResponse({'success': False, 'error': 'La temática ya está registrada'})
            else:
                tematica = Tematicas(nombreTematica=nueva_tematica)
                tematica.save()
                return JsonResponse({'success': True, 'message': 'Temática registrada correctamente'})
        except Exception as e:
            return JsonResponse({'success': False, 'error': f'Error al registrar la temática: {str(e)}'})
    return JsonResponse({'success': False, 'error': 'Solicitud no válida'})

def registrarPedidoCliente(request):
    if request.method == 'POST':
        estado = False
        mensaje = ""
        try:
...
def vistaPedidosCliente(request):
    if request.user.is_authenticated:
        if request.user.groups.filter(name='Cliente').exists():
            cliente_id = request.user.id
            misPedidos = Pedidos.objects.filter(userPedido=cliente_id)
            return render(request, "cliente/listaPedidos.html",{"misPedidos": misPedidos})
        else:
            retorno = {"mensaje": "No tienes permiso para ingresar a esta vista"}
    else:
        retorno = {"mensaje": "Debe ingresar con sus credenciales"}
